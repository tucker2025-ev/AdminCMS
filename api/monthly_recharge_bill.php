...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($data['action'])) {
    ...
    if ($action === 'save_monthly_bill') {
        ...
        foreach ($cpo_items as $item) {
            ...
            $stmt = $connect->prepare("INSERT INTO service_list (cpo_id, invoice_id, fee_date, due_date, description, quantity, rate, sub_total, gst, grand_total, status, entry_time, actions, remaining) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, ?)");
        ...
        $result = $connect->query("SELECT * FROM recharge_bill_monthly WHERE status = 'Y' ORDER BY id DESC");
        $bill_list = [];
